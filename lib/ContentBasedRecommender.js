    ...
    const bigrams = NGrams.bigrams(tokens)
      .filter(
        (bigram) =>
          ...
          bigram.length ===
          this._removeStopwords(bigram, vietnamese_stopwords_list).length
      )
      .map((bigram) =>
        // stem the tokens
    ...
    const trigrams = NGrams.trigrams(tokens)
      .filter(
        (trigram) =>
          // filter terms with stopword
          trigram.length ===
          this._removeStopwords(trigram, vietnamese_stopwords_list).length
      )
      .map((trigram) =>
        // stem the tokens
  ...
  _calculateSimilarities(documentVectors, options) {
    ...
    for (let i = 0; i < documentVectors.length; i += 1) {
      ...
      for (let j = 0; j < i; j += 1) {
        ...
        // console.log("similarity", similarity, vi, vj);
